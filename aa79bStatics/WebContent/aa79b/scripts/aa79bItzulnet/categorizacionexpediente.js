...
//					if(!isObjectInArray(jsonObject.listaCategorizacionExpedienteInicial, metadatosBaja[i])){
...
function seleccionarEtiquetaExpediente(obj, etiquetas) {
									...
									event.preventDefault();
						 			...
						 			if(cambios){
						 				$.rup_messages("msgConfirm", {
						 					...
						 					OKFunction: function(){
						 						limpiarMetadatosList();
						 						$("#buscadorEtiquetasExpediente").remove();
		    						 			volverDetalleExp();
		    						 			cambios = false;
		    						 			jsonObject.listaCategorizacionExpedienteInicial = [];
						 					}
			...
			},
			{
				...
				,click : 
					 function(event){
						...
						$("#capasModalesDiv")
								.append(
										'<div id="buscadorEtiquetasExpediente" style="display:none"></div>');
						$("#buscadorEtiquetasExpediente").buscador_etiquetas({
							//destino : 'P',
							multiselect : false,
							callback : seleccionarEtiquetaExpediente
						});
						$("#buscadorEtiquetasExpediente").buscador_etiquetas("open");
...
//					if(jsonObject.listaCategorizacionExpedienteInicial.length===0){
//						jsonObject.listaCategorizacionExpedienteInicial.push({
//							anyo: $('#anyo_categorizacion_expediente').val(),
//							numExp: $('#numExp_categorizacion_expediente').val(),
//							idMetadato: idMetadato
//						});
...
//						    if(jsonObject.listaCategorizacionExpedienteInicial[i].anyo.toString() === $('#anyo_categorizacion_expediente').val() 
//						    		&& jsonObject.listaCategorizacionExpedienteInicial[i].numExp.toString() === $('#numExp_categorizacion_expediente').val()
//						    		&& jsonObject.listaCategorizacionExpedienteInicial[i].idMetadato.toString() === idMetadato) {
//						    	jsonObject.listaCategorizacionExpedienteInicial.splice(i,1);
//						    	$("#desc_metadato_"+idMetadato).css('background-color', '#ececec');
//						    	borrado = true;
//						    	return false;
