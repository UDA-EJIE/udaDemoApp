...
public class ExperimentalController {
	...
	/**
	 * SERVICIOS NECESARIOS: - Comarca - Localidad - DepartamentoProvincia
	 */
	@Autowired
	...
	public @ResponseBody Object getAll(
		...
		Comarca filterComarca = new Comarca(code, descEs, descEu, css,
				new Provincia());
		...
		if (request.getHeader("JQGridModel") != null
			...
			List<Comarca> comarcas = this.comarcaService.findAll(filterComarca,
					pagination);
	...
	public void removeMultiple(
			@RequestBody ArrayList<ArrayList<String>> comarcaIds) {
		...
		for (ArrayList<String> comarcaId : comarcaIds) {
			...
			Comarca comarca = new Comarca();
			comarca.setCode(ObjectConversionManager.convert(iterator.next(),
	...
	public @ResponseBody Object getAll(
			...
			@RequestParam(value = "css", required = false) String css,
			HttpServletRequest request) {
		Localidad filterLocalidad = new Localidad(code, descEs, descEu, css,
				new Comarca(comarcaCode, null, null, null, null));
		Pagination pagination = null;
		if (request.getHeader("JQGridModel") != null
				&& request.getHeader("JQGridModel").equals("true")) {
			pagination = new Pagination();
			...
			pagination.setSort(request.getParameter("sidx"));
			pagination.setAscDsc(request.getParameter("sord"));
			List<Localidad> localidads = this.localidadService.findAll(
					filterLocalidad, pagination);
	...
	public @ResponseBody Long getAllCount(
			@RequestParam(value = "localidad", required = false) Localidad filterLocalidad,
			HttpServletRequest request) {
	...
	public @ResponseBody Localidad editLocalidad(
	...
	public @ResponseBody Localidad addLocalidad(@RequestBody Localidad localidad) {
	...
	public void removeMultipleLocalidad(
		...
		for (ArrayList<String> localidadId : localidadIds) {
			...
			Localidad localidad = new Localidad();
			localidad.setCode(ObjectConversionManager.convert(iterator.next(),
					java.math.BigDecimal.class));
	...
	public @ResponseBody Localidad getByIdLocalidad(
			@PathVariable BigDecimal codeLocalidad) {
		Localidad localidad = new Localidad();
	...
	// @Json(mixins={@JsonMixin(target=Usuario.class,
	// mixin=UsuarioMixIn.class)})
	...
	public @ResponseBody LogModel edit(@Validated @RequestBody LogModel log) {
		...
		JmsUtils.enviarJMS(log, "x21a.x21aConnectionFactory",
				"x21a.x21aJMSCacheTopic", null);
	...
	public @ResponseBody List<Buzones> getBuzonesNivel3(
			HttpServletRequest request,
	...
	public List<Buzones> getPersonasNombre(HttpServletRequest request,
		...
		try {
			...
			sesion = miApi.n38ItemObtenerPersonas("displayname=*" + nombre
					+ "*");// uid
