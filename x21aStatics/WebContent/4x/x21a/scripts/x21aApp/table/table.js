...
//			plugins.url = "/table";
		...
		/*    plugins.responsive =  {
		    ...
		    plugins.filterForm = '#table_filter_form';
			...
			if(localStorage.plugins.indexOf('buttons') > -1){
				...
				if(localStorage.plugins !== undefined && localStorage.plugins.indexOf('multiPart') > -1){
					...
					var optionButtonEdit = {
							...
							id: 'tableeditMultiPart_1', // Campo obligatorio si se quiere usar desde el contextMenu
					...
					$('#table_detail_div').on( "dialogbeforeclose", function( event, ui ) {
						var dt = $('#table').DataTable();
						var ctx = dt.context[0];
						if(ctx.oInit.formEdit !== undefined && ctx.oInit.formEdit.multiPart){
							console.log('---Trigger--- ' + event.type);
		...
		$("#table_aplicar").click(function(){
			if(localStorage.plugins === undefined){
				localStorage.plugins = '';
			}
			
			var selectionType = $("input[name = table_seleccionTabla]:checked")[0].id;
			
			$.each($("#table_tableConfiguration .pluginsControl input"), function() {
