...
(function ($) {
	...
	$.fn.rup_maint("extend", {
		newElement : function () {//Muestra el formulario de adición y pone el mantenimiento en modo alta.
			...
			return this.each(function () {
				...
				if (this.prop.jQueryGrid.rup_grid("isEditable")) {
					...
					this.prop.detailForm.data('initialData', this.prop.detailForm.serialize());
		...
		resetForm: function(form){
			...
			$("[ruptype='autocomplete']").each(function (index, element) {
				$(element).val("");
			});
		},
		updateDetailPagination : function (rowPos, totalElements) {
			...
			return this.each(function () {
				//$("#"+$(this).attr("id")).next().find("#rup_maint_selectedElements").text($.jgrid.format($.rup.i18nParse($.rup.i18n.base,"rup_grid.defaults.detailForm_pager"),rowPos,totalElements));
				$("#rup_maint_selectedElements").text($.jgrid.format($.rup.i18nParse($.rup.i18n.base,"rup_grid.defaults.detailForm_pager"),rowPos,totalElements));
...
//						mant.prop.jQueryGrid.rup_grid("delRowData",mant.prop.jQueryGrid.rup_grid("getGridParam",'selrow'));
						...
						for (var i = 0; i < aux.length; i++) {
							if (aux[i].value !== "") {
								...
								for (var j=0; j<parseableChars.length; j++){
									if (fieldName.indexOf(parseableChars[j])!== -1){
		...
		search : function (page) {//Lanza la busqueda del mantenimiento obteniendo los datos del formulario de busqueda
			...
			return this.each(function () {
				...
				this.prop.jQueryGrid.rup_grid("setGridParam", {url: $.rup_utils.setNoPortalParam(this.prop.jQueryGrid[0].rup_gridProps.url), datatype: 'json', mtype: "GET", page: (page ? page : "rup")});
...
//			}
		...
		_checkSelectedElements: function (okCallback){
			var self = this, prop = self[0].prop;
			var selectedRows=prop.selectedRows;
		...
		_ondblClickRow: function (rowid, iRow, iCol, e) {
			...
			if (!prop.jQueryGrid.rup_grid("isMultiselect") && !prop.jQueryGrid.rup_grid("isEditable")) {
				prop.MODO = "edit";
				self.rup_maint("editElement", rowid);
				$("#"+rowid).click();//Si tiene detalle que se carguen datos en doble click
			}
		},
		_saveMaintSuccess: function (xhr, ajaxOptions, saveAndRepeat, rowId, aftersavefunc) {
			...
			if (prop.jQueryGrid.rup_grid("isEditable")) {
				...
				if (saveAndRepeat) { //si es guardar y repetir
					...
					prop.detailForm.data('initialData', prop.detailForm.serialize());
				} else {
					self.rup_maint("resetForm", prop.detailForm);
					if (prop.showMessages) {
						if (prop.MODO === 'new') { //Mostrar los mensajes dependiendo el modo
...
//						t[0].prop.detailForm.data('initialData', t[0].prop.detailForm.serialize());
			...
			prop.toolbar.defaultFunctionEditButton = function () {
				var rowid = jqGrid.rup_grid('getGridParam', 'selrow'), page = jqGrid.rup_grid('getGridParam', 'page'),firstPage=null, firstElemObj;
				if (jqGrid.rup_grid('isMultiselect')) {
			 ...
			 * VALIDACIONES
			 */
			var colModel,jsonAux, configValidationRules, validationConfig;
...
//			if ($.isEmptyObject(jsonConfigValidaciones)===false){
...
//				}
			...
			if(jqGrid[0].rup_gridProps.fluidBaseLayer===null){
				jqGrid[0].rup_gridProps.fluidBaseLayer = "#"+$(self).attr("id");
			}
			/* HACK IE */
			if ($.rup.browser.isIE){ 
				jqGrid[0].rup_gridProps.windowWidth=$(window).width();
			}
			/* FIN HACK IE */
				$(window).resize(function(a, a, a){
					/* HACK IE */
					if(!$.rup.browser.isIE || ($.rup.browser.isIE && jqGrid[0].rup_gridProps.windowWidth!==$(window).width())){
						if ($.rup.browser.isIE){
							jqGrid[0].rup_gridProps.windowWidth=$(window).width();
						}
						/* FIN HACK IE */
						jqGrid.fluidGrid({
							base : jqGrid[0].rup_gridProps.fluidBaseLayer,
							offset : jqGrid[0].rup_gridProps.offset
						});
						//Definicion del evento para el grid. Sin esto el efecto tiene retardo en el grid.
						...
						//Capa contenedora
						$("#"+prop.name).width($(window).width()*prop.contentPercentage);
						
						//Comprobar tamaños: espacio de visualización y el tamaño de la tabla
						var newWidth = $("#"+prop.name).width()-prop.fluidOffset,
							oldWidth = $(jqGrid)[0].p.gridWidth;
						if (newWidth <= oldWidth){
							//Redimensionar
							$("#"+prop.name).children().width(newWidth);
							$(jqGrid).setGridWidth(newWidth);
						} else {
							//Tamaño inicial
							$("#"+prop.name).children().width(oldWidth);
							$(jqGrid).setGridWidth(oldWidth);
						}
							
						//Feedback
						var padding_left = parseInt(prop.feedback.css("padding-left")),
						    padding_right = parseInt(prop.feedback.css("padding-right"));
						$("#feedback_"+prop.name).css("width", $(jqGrid).rup_grid("getGridParam", "width") - (padding_left+padding_right));
						//Toolbar
						if (prop.toolbar.autoAjustToolbar) {
							prop.toolbar.self.css("width", prop.jQueryGrid.rup_grid("getGridParam", "width") - 5);//-5 para ajustar el ancho
						}
				...
				//Guardar tamaño predefinido para tabla 
				$(jqGrid)[0].p.gridWidth = $(jqGrid)[0].p.width;
				
				//Si el espacio de visualización es menor menor que el tamaño de la tabla se redimensiona
				var elementWidth = $("#"+prop.name).width()-prop.fluidOffset;
				if (elementWidth <= $(jqGrid)[0].p.gridWidth){
					$("#"+prop.name).children().width(elementWidth);
					$(jqGrid).setGridWidth(elementWidth);
					//Feedback
					var padding_left = parseInt(prop.feedback.css("padding-left")),
						padding_right = parseInt(prop.feedback.css("padding-right"));
					$("#feedback_"+prop.name).css("width", $(jqGrid).rup_grid("getGridParam", "width") - (padding_left+padding_right));
	...
	$.fn.rup_maint("extend",{
		...
		_editNextElement:function(){
			...
			if (page!==nextElement.page){
				...
				jqGrid.rup_grid("reloadGrid", reloadGrid_callback_next);
			} else {
				reloadGrid_callback_next();
			}
			
			function reloadGrid_callback_next(){	
				if (!jqGrid.rup_grid("isMultiselect")) {
					if (pageChanged){
		...
		_getDetailUrl: function(rowId){
			...
			if (prop.jQueryGrid.rup_grid("isMultiselect") === false) {
				...
				if (self._getPrimaryKeysForRow(rowId) !== "/false"){
					...
					detailURL = detailURL + "/" + selectedRows[0];
				}
				return detailURL;
		...
		_getLastElement:function(page){
			...
			lastPage = $.data(jqGrid[0], "numPags");
