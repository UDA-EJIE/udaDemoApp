		...
		option: function (optionName, value, aux){
			...
			if (optionName === "source"){
				if (typeof value === "object"){
					...
					$(this).autocomplete("option" , optionName , this._sourceLOCAL);
					$(this).autocomplete("option" , "minLength" , settings.minLength !== undefined?settings.minLength:0);
				} else {
					//REMOTO
					settings.sourceParam = aux;
					//Nos aseguramos que el número mínimo de teclas para búsquedas sea 3
					$(this).autocomplete("option" , optionName , this._sourceREMOTE);
					$(this).autocomplete("option" , "minLength" , settings.minLength>3?settings.minLength:3);
				}
				settings.data = value;
