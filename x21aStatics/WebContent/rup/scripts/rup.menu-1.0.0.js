...
(function ($) {
	...
	$.rup_menu.extend({
			...
			for (var i = json.length; i--; ) {
				...
				if (element.url !== undefined){
					...
					menu.prepend($('<li>').append(
						$('<a>').text(json_i18n[element.i18nCaption]).attr('href', $.rup.CTX_PATH+element.url).css('text-decoration', 'underline')
					));
				} else {
					//Si no tiene enlace es submenu
					menu.prepend($('<li>').append(
						$('<a>').text(json_i18n[element.i18nCaption]).css('cursor','default')
					));
		...
		parseJSON_horizontal: function (json, json_i18n, menu){
			...
			for (var i = json.length; i--; ) {
				element = json[i];
				menu.prepend($('<a>').text(json_i18n[element.i18nCaption]));
				//Añadir enlace en caso de que tenga
				if (element.url !== undefined){
					menu.children('a:first-child').attr('href', $.rup.CTX_PATH+element.url);
				}
			}
			delete element;
		} 
    ...
    $.fn.rup_menu = function (properties) {
		return this.each ( function() {		
			...
			/** MENU HORIZONTAL **/
				...
				t.children("a").button()
					.click(function(){
						...
						if(submenu.parent().find("ul:visible").length>0){
							submenu.parent().find("ul:visible").data("nestedmenu").active.data("menu").previous(); //resalto primera opción
							submenu.parent().find("ul:visible").hide();
							submenu.parent().find(".ui-state-focus").removeClass("ui-state-focus"); //quitar estilo botón seleccionado
						}
						//Mostrar actual
						if (display==='none'){
				...
				$.each(t.children("a"), function (index, element){
					if ($(element).attr('href') === undefined){
						...
						$(element)
								...
								.append($('<div>')
								.attr('id', divID));
						$('#'+divID).rup_menu({ display: 'vertical', json: json[index].submenu, json_i18n:json_i18n });
			...
			/** MENU VERTICAL */
				...
				//Se cambia la capa inicial por su UL hijo ya que así es como lo requiere ui.menu
				$('#'+settings.id).replaceWith($('#'+settings.id+ " ul:first-child").attr('id', settings.id));
				$('#'+settings.id).addClass("rup_menu");
