CREATE OR REPLACE VIEW X54JAPI_PERSONAL_IZO AS 
...
FROM X54J44T00
JOIN (SELECT DNI_042
    , MAX(DECODE(CPER_043, 1, 'S', 'N')) AS TECNICO_GESTOR
    , MAX(DECODE(CPER_043, 2, 'S', 'N')) AS ASIGNADOR
    , MAX(DECODE(CPER_043, 3, 'S', 'N')) AS TRADUCTOR
		, MAX(DECODE(CPER_043, 4, 'S', 'N')) AS INTERPRETE
		FROM X54J42T00
