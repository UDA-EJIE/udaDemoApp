CREATE OR REPLACE FUNCTION HORAS_ENTRE_FECHAS_ATENCION(FECHA_INI IN DATE, FECHA_FIN IN DATE) RETURN VARCHAR IS
RETORNO VARCHAR2(10):='';
BEGIN
WITH FECHAS AS
...
, HORARIOS AS
    (SELECT DIA_SEMANA, IND_VERANO_021 AS HORARIO, TIPO
    , CASE WHEN TIPO = 'M' THEN DECODE(DIA_SEMANA, 5, HORA_INI_V_M_021, HORA_INI_LJ_M_021) ELSE DECODE(DIA_SEMANA, 5, HORA_INI_V_T_021, HORA_INI_LJ_T_021) END AS HORA_INI
    , CASE WHEN TIPO = 'M' THEN DECODE(DIA_SEMANA, 5, HORA_FIN_V_M_021, HORA_FIN_LJ_M_021) ELSE DECODE(DIA_SEMANA, 5, HORA_FIN_V_T_021, HORA_FIN_LJ_T_021) END AS HORA_FIN
    FROM AA79B21S01
...
, CALENDARIO AS
    ...
    ORDER BY DIA)
SELECT FORMAT_HORA_MINUTOS(NVL(SUM( ROUND((FIN-INICIO) * 24, 2)),0))
INTO RETORNO
FROM CALENDARIO
