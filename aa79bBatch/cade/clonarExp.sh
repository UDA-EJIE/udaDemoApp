...
NOMBRE_FICHERO_LOGS=aa79bConarExpediente_${DATE}_${TIME}
...
echo " "
echo "---------------------------------------------------------------------------------------------------------" >> $FICHERO_LOGS
echo "Comienzo del proceso batch java AA79B (Clonar expediente) ....." >> $FICHERO_LOGS
date >>  $FICHERO_LOGS
echo " " >> $FICHERO_LOGS
...
export DB_DRIVER
#Definimos el id de la clonacion (tabla 0A8) como parametro
IDCLONACION=$1
...
$JAVA_HOME/bin/java -Dfile.encoding=ISO-8859-15 -d64 -cp $CLASSPATH_JAVA_PROCESO $DIR_CLASE_MAIN.$CLASE_JAVA "$USUARIO_XLNETS" "$PASSWORD" "$DB_DRIVER" "$DNS" "$IDCLONACION" >> $FICHERO_LOGS
	
if [ ${?} != 0 ]; then	
	echo "El proceso java para clonar expediente no ha finalizado correctamente"
	echo "Comprobar fichero de datos ($FICHERO_LOGS) para detectar errores"
	exit 1
else
	echo "El proceso java para clonar expediente ha finalizado correctamente"
fi
