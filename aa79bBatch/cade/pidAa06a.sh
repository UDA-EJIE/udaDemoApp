...
NOMBRE_FICHERO_LOGS=aa79bSubirDocumentosAa06aPid_${DATE}_${TIME}
...
CLASE_JAVA=SubirDocumentosAa06aPid
export CLASE_JAVA
...
echo " "
echo "---------------------------------------------------------------------------------------------------------" >> $FICHERO_LOGS
echo "Comienzo del proceso batch java AA79B (Subir documentos al PID) ....." >> $FICHERO_LOGS
date >>  $FICHERO_LOGS
echo " " >> $FICHERO_LOGS
...
$JAVA_HOME/bin/java -Dfile.encoding=ISO-8859-15 -d64 -cp $CLASSPATH_JAVA_PROCESO $DIR_CLASE_MAIN.$CLASE_JAVA "$USUARIO_XLNETS" "$PASSWORD" "$DB_DRIVER" "$DNS" "$ANYO" "$NUMEXP" >> $FICHERO_LOGS
