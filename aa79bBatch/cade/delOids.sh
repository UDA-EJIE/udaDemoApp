...
NOMBRE_FICHERO_LOGS=aa79bBorrarOidsAuxiliares_${DATE}_${TIME}
...
CLASE_JAVA=BorrarOidsAuxiliares
export CLASE_JAVA
...
echo " "
echo "---------------------------------------------------------------------------------------------------------" >> $FICHERO_LOGS
echo "Comienzo del proceso batch java AA79B (Borrar Oids auxiliares) ....." >> $FICHERO_LOGS
date >>  $FICHERO_LOGS
echo " " >> $FICHERO_LOGS
...
if [ ${?} != 0 ]; then	
	echo "El proceso java no ha finalizado correctamente"
	echo "Comprobar fichero de datos ($FICHERO_LOGS) para detectar errores"
	exit 1
else
	echo "El proceso java ha finalizado correctamente"
fi
