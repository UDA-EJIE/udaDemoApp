...
NOMBRE_FICHERO_LOGS=aa79bAvisosAuditoriasIZO_${DATE}_${TIME}
...
CLASE_JAVA=AvisosAuditoriasIZO
export CLASE_JAVA
...
echo " "
echo "---------------------------------------------------------------------------------------------------------" >> $FICHERO_LOGS
echo "Comienzo del proceso batch java AA79B (Avisos Auditorias al IZO ) ....." >> $FICHERO_LOGS
date >>  $FICHERO_LOGS
echo " " >> $FICHERO_LOGS
...
if [ ${?} != 0 ]; then	
	echo "El proceso java para enviar avisos de auditoria al IZO no ha finalizado correctamente"
	echo "Comprobar fichero de datos ($FICHERO_LOGS) para detectar errores"
	exit 1
else
	echo "El proceso java para enviar avisos de auditoria al IZO ha finalizado correctamente"
fi
