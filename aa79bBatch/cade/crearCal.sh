...
NOMBRE_FICHERO_LOGS=aa79bCrearCalendariAnyoSig_${DATE}_${TIME}
...
CLASE_JAVA=CrearCalendarioAnyoSig
export CLASE_JAVA
...
if [ ${?} != 0 ]; then	
	echo "El proceso java para crear el calendario del año próximo no ha finalizado correctamente"
	echo "Comprobar fichero de datos ($FICHERO_LOGS) para detectar errores"
	exit 1
else
	echo "El proceso java para crear el calendario del año próximo ha finalizado correctamente"
fi
