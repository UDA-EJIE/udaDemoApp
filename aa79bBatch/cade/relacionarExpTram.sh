...
NOMBRE_FICHERO_LOGS=aa79bRelacionarExpedientesTramitagune_${DATE}_${TIME}
...
CLASE_JAVA=ActualizarExpRelacionados
export CLASE_JAVA
...
echo " "
echo "---------------------------------------------------------------------------------------------------------" >> $FICHERO_LOGS
echo "Comienzo del proceso batch java AA79B (Actualizar expedientes relacionados) ....." >> $FICHERO_LOGS
date >>  $FICHERO_LOGS
echo " " >> $FICHERO_LOGS
...
if [ ${?} != 0 ]; then	
	echo "El proceso java para verificar certificados de parte no ha finalizado correctamente"
	echo "Comprobar fichero de datos ($FICHERO_LOGS) para detectar errores"
	exit 1
else
	echo "El proceso java para verificar certificados de parte ha finalizado correctamente"
fi
