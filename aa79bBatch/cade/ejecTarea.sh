...
NOMBRE_FICHERO_LOGS=aa79bEjecutarTarea_${DATE}_${TIME}
...
echo " "
echo "---------------------------------------------------------------------------------------------------------" >> $FICHERO_LOGS
echo "Comienzo del proceso batch java AA79B (Ejecutar tarea) ....." >> $FICHERO_LOGS
date >>  $FICHERO_LOGS
echo " " >> $FICHERO_LOGS
...
export DB_DRIVER
#Definimos el id de la tarea como parametro
IDTAREA=$1
...
$JAVA_HOME/bin/java -Dfile.encoding=ISO-8859-15 -d64 -cp $CLASSPATH_JAVA_PROCESO $DIR_CLASE_MAIN.$CLASE_JAVA "$USUARIO_XLNETS" "$PASSWORD" "$DB_DRIVER" "$DNS" "$IDTAREA" >> $FICHERO_LOGS
