...
/**
 * The type ExcelInformeEntidad.
 *
 */
public class ExcelInforme extends ExcelContentMetodos {
	...
	public void addColumna(String literal, String propiedad, Integer ancho) {
		this.addColumna(literal, propiedad, ancho, false);
	}

	public void addColumna(String literal, String propiedad, Integer ancho, Boolean ajustar) {
		this.addColumna(literal, "", propiedad, ancho, false);
	}
	...
	public void procesarContenido(XSSFWorkbook libro, XSSFSheet hoja) {
		...
		final XSSFCellStyle celdaTabla = ExcelEstilo.getEstilo(libro, ExcelEstilo.CONTENT, null);
		...
		if (!this.criterios.isEmpty()) {
			for (Criterio crit : this.criterios) {
				...
				celda = this.crearCelda(hoja, col, y, crit.getAnchoValor(),
						Reports.retrieveObjectValue(this.criterio, crit.getPropiedad()), celdaTabla);
				col += crit.getAnchoValor() + 1;
				if (crit.getSaltoLinea()) {
					y++;
					col = x;
				}
			}
