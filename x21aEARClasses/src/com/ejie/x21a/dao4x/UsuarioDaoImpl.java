...
public class UsuarioDaoImpl implements UsuarioDao {
    ...
    */
	...
	@Override
	public List<TableRowDto<Usuario>> reorderSelection(Usuario usuario, Pagination<Usuario> pagination,
			Boolean startsWith) {
		
		// SELECT
		StringBuilder sbSQL = new StringBuilder("SELECT  t1.ID ID,t1.NOMBRE NOMBRE,t1.APELLIDO1 APELLIDO1,t1.APELLIDO2 APELLIDO2,t1.EJIE EJIE,t1.FECHA_ALTA FECHAALTA,t1.FECHA_BAJA FECHABAJA ");
		...
		StringBuilder sbReorderSelectionSQL = PaginationManager.getReorderQuery(sbSQL, pagination, Usuario.class, filterParamList, "ID");
		
		return this.jdbcTemplate.query(sbReorderSelectionSQL.toString(), new RowNumResultSetExtractor<Usuario>(this.rwMapPK, pagination), filterParamList.toArray());
	}
	
	
	
	@Override
	public List<TableRowDto<Usuario>> search(Usuario filterParams, Usuario searchParams, Pagination<Usuario> pagination, Boolean startsWith) {
		...
		String searchSQL = (String) mapaWhereSearch.get("query");
		...
		StringBuilder sbReorderSelectionSQL = PaginationManager.getSearchQuery(sbSQL, pagination, Usuario.class, filterParamList, searchSQL, searchParamList, "ID");
...
//		StringBuilder searchQuery = new StringBuilder("SELECT z.* FROM(SELECT rownum TABLELINE, v.* FROM(");
//		searchQuery.append(query);
//		searchQuery.append(") v ) z ");
		
//		StringBuilder baseQuery = new StringBuilder("SELECT t1.ID ID,t1.NOMBRE NOMBRE,t1.APELLIDO1 APELLIDO1,t1.APELLIDO2 APELLIDO2,t1.EJIE EJIE,t1.FECHA_ALTA FECHAALTA,t1.FECHA_BAJA FECHABAJA "); 
//		baseQuery.append("FROM USUARIO t1 ");
		
//		//Where clause & Params
//		Map<String, ?> mapaWhereSearch = this.getWhereLikeMap(searchParams, startsWith); 
//		StringBuilder whereSearch = new StringBuilder(" WHERE 1=1 ");
//		whereSearch.append(mapaWhereSearch.get("query"));
//		String filtroSearch = whereSearch.toString();
//		String aliasTablaStr = "T1";
//		searchQuery.append(filtroSearch.replaceAll("(?i)"+aliasTablaStr+"\\.", "").trim());
//		
//		params.addAll((List<Object>) mapaWhereSearch.get("params"));
...
//		return (List<Usuario>) this.jdbcTemplate.query(query.toString(), this.rwMap, params.toArray());
//		return this.jdbcTemplate.query(searchQuery.toString(), new RowNumResultSetExtractor<Usuario>(this.rwMap, "id"), params.toArray());
		
//		SELECT * FROM (
//				 SELECT a.* FROM (
//				  SELECT  rownum rnum, t1.ID ID, NOMBRE, APELLIDO1 FROM USUARIO t1  
//				 ) a WHERE 1=1  AND UPPER(APELLIDO1) like '%AMAIA%' ESCAPE  '\' 
//				      AND APELLIDO1 IS NOT NULL ORDER BY id asc
//				)
//								    WHERE ID IN ('1077','1082','1088','1145','1141')
		
	}

	
//	class ReorderSelectionResultSetExtractor<T> implements ResultSetExtractor<List<TableRow<T>>>{
//
//		private RowMapper<T> rowMapper;
//		private Pagination pagination;
//		
//		@Override
//		public List<TableRow<T>> extractData(ResultSet resultSet) throws SQLException,
//				DataAccessException {
//			List<TableRow<T>> listTableRow = new ArrayList<TableRow<T>>();
//			
//			while (resultSet.next()) {
////				TableRow<T> tableRow = new TableRow<T>();
////				tableRow.setRowNum(resultSet.getInt("rnum"));
//////				tableRow.applyPagination(this.pagination);
////				
////				T obj = rowMapper.mapRow(resultSet, resultSet.getRow());
////				tableRow.setObj(obj);
////				tableRow.setId(this.getId(obj));
...
//		}
//		
//		public ReorderSelectionResultSetExtractor(RowMapper<T> rowMapper, Pagination pagination){
...
//	}
	...
	@Override
	public List<JerarquiaDto<Usuario>> findAllLikeJerarquia(Usuario filterUsuario, Pagination pagination) {
		...
		//JERARQUIA
		sbSQL = PaginationManagerJerarquia.getQuery(pagination, sbSQL, mapaWhere, "ID", "ID_PADRE", "NOMBRE", from, from_alias);
//		sbSQL = PaginationManagerJerarquia.getQuery(pagination, sbSQL, mapaWhere, "ID", "ID_PADRE", "NOMBRE", from, from_alias, joins, businessFilters, businessParams);
		...
		//PAGINACIÓN
		if (pagination != null) {
			sbSQL = PaginationManagerJerarquia.getPaginationQuery(pagination, sbSQL);
		}
	...
	@Override
	public Long findAllLikeCountJerarquia(Usuario filterUsuario, Pagination pagination) {
		...
		//JERARQUIA
		StringBuilder sbSQL = PaginationManagerJerarquia.getQueryCount(pagination, mapaWhere, "ID", "ID_PADRE", from, from_alias);
//		StringBuilder sbSQL = PaginationManagerJerarquia.getQueryCount(pagination, mapaWhere, "ID", "ID_PADRE", from, from_alias, joins, businessFilters, businessParams);
	...
	public List<TableRowDto<Usuario>> findAllChild(Usuario filterUsuario, Pagination pagination) {
		...
		//MULTISELECCION
		StringBuilder sbSQL = PaginationManagerJerarquia.getQueryChildren(pagination, mapaWhere, "ID", "ID_PADRE", from, from_alias);
