...
public class ComarcaDaoImpl implements ComarcaDao {
	...
	private RowMapper<Comarca> rwMap = new RowMapper<Comarca>() {
		public Comarca mapRow(ResultSet resultSet, int rowNum) throws SQLException {
               ...
               new Provincia(resultSet.getBigDecimal("ProvinciaCODE"), resultSet.getString("ProvinciaDESCES"), resultSet.getString("ProvinciaDESCEU"), resultSet.getString("ProvinciaCSS"))
			); 
		} 
	};
	...
	private RowMapper<Comarca> rwMapLocalidad = new RowMapper<Comarca>() {
		public Comarca mapRow(ResultSet resultSet, int rowNum) throws SQLException {
           	return new Comarca(
               resultSet.getBigDecimal("CODE"), resultSet.getString("DESCES"), resultSet.getString("DESCEU"), resultSet.getString("CSS"), 
               new Provincia(resultSet.getBigDecimal("ProvinciaCODE"), resultSet.getString("ProvinciaDESCES"), resultSet.getString("ProvinciaDESCEU"), resultSet.getString("ProvinciaCSS")),
               new Localidad(resultSet.getBigDecimal("CODELOCALIDAD"))
			); 
		} 
	};
    ...
    public Comarca update(Comarca comarca) {
		String query = "UPDATE COMARCA SET CODE_PROVINCIA=?, DESC_ES=?, DESC_EU=?, CSS=?, CODE_AYUNTAMIENTO=? WHERE CODE=?";
		Object getProvinciaCodeAux = null;
		Object codeLocalidad = null;
		if (comarca.getProvincia() != null && comarca.getProvincia().getCode() != null) {
			getProvinciaCodeAux = comarca.getProvincia().getCode();
		}
		if (comarca.getLocalidad() != null && comarca.getLocalidad().getCode() != null) {
			codeLocalidad = comarca.getLocalidad().getCode();
		}
		this.jdbcTemplate.update(query, getProvinciaCodeAux, comarca.getDescEs(), comarca.getDescEu(), comarca.getCss(),codeLocalidad, comarca.getCode());
		return comarca;
	}
    ...
    public Comarca find(Comarca comarca) {
		String query = "SELECT t1.CODE CODE, t1.DESC_ES DESCES, t1.DESC_EU DESCEU, t1.CSS CSS, t2.CODE PROVINCIACODE, t2.DESC_ES PROVINCIADESCES, t2.DESC_EU PROVINCIADESCEU, t2.CSS PROVINCIACSS, t1.CODE_AYUNTAMIENTO CODELOCALIDAD FROM COMARCA t1 , PROVINCIA t2  WHERE t1.CODE = ?   AND t1.CODE_PROVINCIA= t2.CODE(+)";
		
		List<Comarca> comarcaList = this.jdbcTemplate.query(query, this.rwMapLocalidad, comarca.getCode());
    ...
    public List<Comarca> findAllLike(Comarca comarca, TableRequestDto tableRequestDto, Boolean startsWith) {
		StringBuilder query = new StringBuilder("SELECT  t1.CODE CODE,t1.DESC_ES DESCES,t1.DESC_EU DESCEU,t1.CSS CSS,t2.CODE PROVINCIACODE,t2.DESC_ES PROVINCIADESCES,t2.DESC_EU PROVINCIADESCEU,t2.CSS PROVINCIACSS, t1.CODE_AYUNTAMIENTO CODELOCALIDAD "); 
        query.append("FROM COMARCA t1 ,PROVINCIA t2 ");
		...
		return (List<Comarca>) this.jdbcTemplate.query(query.toString(), this.rwMapLocalidad, params.toArray());
	}
