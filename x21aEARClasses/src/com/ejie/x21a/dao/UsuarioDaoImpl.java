...
//								    WHERE ID IN ('1077','1082','1088','1145','1141')
	...
	public List<TableRow<Usuario>> search(Usuario filterParams, Usuario searchParams, Pagination pagination, Boolean startsWith) {
		...
		//Where clause & Params
		Map<String, Object> mapaWhere = this.getWhereLikeMap(filterParams, startsWith); 
		StringBuilder where = new StringBuilder(" WHERE 1=1 ");
		where.append(mapaWhere.get("query"));
		query.append(where);
		...
		StringBuilder searchQuery = new StringBuilder("SELECT z.* FROM(SELECT rownum rnum, v.* FROM(");
...
//		return (List<Usuario>) this.jdbcTemplate.query(query.toString(), this.rwMap, params.toArray());
		return this.jdbcTemplate.query(searchQuery.toString(), new SearchResultSetExtractor<Usuario>(this.rwMap, pagination){

			@Override
			public String getId(Usuario row) {
				return row.getId();
			}
...
//								    WHERE ID IN ('1077','1082','1088','1145','1141')
	...
	abstract class SearchResultSetExtractor<T> implements ResultSetExtractor<List<TableRow<T>>>{
...
//				tableRow.applyPagination(this.pagination);
		...
		public SearchResultSetExtractor(RowMapper<T> rowMapper, Pagination pagination){
			this.rowMapper = rowMapper;
			this.pagination = pagination;
		}
		
		public abstract String getId(T row);
	}
