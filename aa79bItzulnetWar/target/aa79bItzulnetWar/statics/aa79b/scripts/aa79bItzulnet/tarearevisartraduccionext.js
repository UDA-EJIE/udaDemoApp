			...
			$.ajax({
			     ...
			     ,cache: false
			   	 ,success:function(extensionValida){
			   		 desbloquearPantalla();
			   		$('#docsRevisarTradExt').rup_table("filter");
...
function volcarContenidoInfoDocFinal(docAdjunto){
	var contenidoCelda= '<button id="pidButtonFinal_'+docAdjunto.idDocOriginal+'" type="button" onclick="clickPidButtonFinal('+docAdjunto.idDocOriginal+',\''+docAdjunto.extension+'\',\''+expedienteConfidencial+'\',\''+docAdjunto.nombre+'\')" class="btnPID ui-button ui-corner-all ui-widget" style="margin-top:-4px">'+$.rup.i18nParse($.rup.i18n.app,"label.adjuntarFichero")+'</button>';	
	if (docAdjunto.idFichero != null){
		contenidoCelda+='<div style="display: inline-table;">';
		contenidoCelda+= '<a href="#" onclick="eliminarDocumentoRevisado('+docAdjunto.idDocOriginal+')" class="flotaIzda rup-enlaceCancelar" >'+$.rup.i18n.app.comun.eliminar+'</a>';
		contenidoCelda+='</div>';	
		contenidoCelda+='<div style="display: table-row;">';
		contenidoCelda+= '<a href="#" onclick="descargarDocumentoGeneral('+ docAdjunto.idFichero +')" class="flotaIzda">';
...
function generarNoConformidad(){
	...
	if ($("#generarNoConformidad_detail_form").valid()){
		...
		$.ajax({
		   	 type: 'POST'		   	 
		   	 ,url: '/aa79bItzulnetWar/ejecuciontarea/generarNoConformidad'
		 	 ,dataType: 'json'
		 	 ,contentType: 'application/json'
	 		 ,data: $.toJSON({
		   			"tarea" : Tareas,
...
function mostrarInfoGroupBy(cellval, opts, rowObject){
    ...
    if("docsRevisarTradExtghead_" !== prefix){
...
// FIN Subida del fichero asociado a la tarea de notificacion de corrección al proveedor
...
function cargarDatosLote() {
	...
	$.ajax({
		...
		success : function(data) {
			if (data.lotes != null) {
				$("#indFacturable_form_tradExt").val(true);
				$("#retraso_tradExt").text(data.lotes.porPenalizacion + "%");
				$("#retrasoVal_tradExt").val(data.lotes.porPenalizacion);
				
				if (data.datosPagoProveedores != null) {
					if (data.datosPagoProveedores.indPenalizacion === "S") {
						$("#porPenalizacionSwitch_tradExt").bootstrapSwitch('setState', true);
						$("#porcentajePenalizacion_tradExt").val(data.datosPagoProveedores.porPenalizacion);
					} else {
						$("#porPenalizacionSwitch_tradExt").bootstrapSwitch('setState', false);
					}

					if (data.datosPagoProveedores.nivelCalidad != null) {
						$("#calidad_combo_tradExt").rup_combo("setRupValue", data.datosPagoProveedores.nivelCalidad);
					}
				} else {
					$("#porPenalizacionSwitch_tradExt").bootstrapSwitch('setState', false);
				}
				...
				$("#datosProveedor").hide();
				$("#indFacturable_form_tradExt").val(false);
			}
		}
	});
}
...
function porPenalChange(){
	var valor = parseInt($("#porcentajePenalizacion_tradExt").val());
	if(valor!= '' && valor < 0 || valor > 100){
		...
		$("#porcentajePenalizacion_tradExt").addClass("error");
	} else {
		$("#porcentajePenalizacion_tradExt").removeClass("error");
	}
}
...
jQuery(function($) {
	...
	$('#docsRevisarTradExt_feedback').rup_feedback(opcionesFeedbacks);
	...
	$("#ejecutarTareaDialog_form").rup_form({
		...
		beforeSubmit: function(){
			...
			$("#indPenalizacion_form_tradExt").val($("#porPenalizacionSwitch_tradExt").is(':checked') ? datos.activo.si : datos.activo.no)
			$("#porPenalizacion_form_tradExt").val($("#porcentajePenalizacion_tradExt").val());	
			$("#nivelCalidad_form_tradExt").val($("#calidad_combo_tradExt").rup_combo("getRupValue"));
		},
		success: function(){
					 ...
					 function(e){
						...
						if (comprobarTareaNoEjecutada(idTarea)){
		                    ...
		                    if(valor!= '' && valor < 0 || valor > 100){
		                		$('#ejecutarTareaDialog_feedback').rup_feedback("set", $.rup.i18n.app.validaciones.porcentajeMenorCien, "error");
		                		$("#porcentajePenalizacion_tradExt").addClass("error");
		                    } else if ($("#porPenalizacionSwitch_tradExt").is(':checked') && parseInt($('#porcentajePenalizacion_tradExt').val()) > parseInt($("#retrasoVal_tradExt").val())) {
		                    	$('#ejecutarTareaDialog_feedback').rup_feedback("set", $.rup.i18n.app.validaciones.porcentajePenalMayorQueLote, "error");
		                		$("#porcentajePenalizacion_tradExt").addClass("error");
		                    } else {
		                    	if ($('#porcentajePenalizacion_tradExt').val() === '' && $("#porPenalizacionSwitch_tradExt").is(':checked')) {
		                    		$("#porcentajePenalizacion_tradExt").val(0);
		                    	}
			                    ocultarObservaciones(e);
	...
	$("#docsRevisarTradExt_toolbar").rup_toolbar({
		buttons:[
			{
	...
	$("#tabsModalRevTradExt").rup_tabs({
		tabs : [
			{i18nCaption: labelDocusRevisarTradExt, layer:"#pestDocsRevisarTradExt"},
	...
	/* RUP_TABLES */
	$("#docsRevisarTradExt").rup_table({
		url: "/aa79bItzulnetWar/ejecuciontarea/docsRevisionTraduccionExterna",
		colNames: [
			"",
			"",
			"",
        ...
        usePlugins:[
     	...
     	loadComplete: function(){ 
     		habilitarPager('docsRevisarTradExt');
     		$('#pg_docsRevisarTradExt_pager').find('td.pagControls select').hide();
        },
     	grouping: true,
		groupingView: {
	...
	$("#docsXliff").rup_table({
		url: "/aa79bItzulnetWar/ejecuciontarea/docsRevTradExt/xliff",
		toolbar:{
	...
	$("#porPenalizacionSwitch_tradExt").bootstrapSwitch()
	...
	$("#porPenalizacionSwitch_tradExt").on('switch-change', function(event, state) {
        if (state.value){ //Activando
            $("#porcentajePenalizacion_tradExt").attr("disabled", false);
        }else{
            $("#porcentajePenalizacion_tradExt").attr("disabled", true);
            $('#porcentajePenalizacion_tradExt').val("");
        }
        porPenalChange();
    });
	
    $("#porcentajePenalizacion_tradExt").change(function(){
    	porPenalChange();
	});
    
    $("#calidad_combo_tradExt").rup_combo({
		...
		,open: function(){
			jQuery('#calidad_combo_tradExt-menu').width(jQuery('#calidad_combo_tradExt-button').innerWidth());
		}
	});
