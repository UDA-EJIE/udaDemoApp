  ...
  de la aplicación pueda cerrar la capa manualmente.
     ...
     */
		_create: function _create() {
			var opciones = this.options;
			opciones._idFeedback = this.element.addClass('rup-feedback ui-widget ui-widget-content ui-corner-all').addClass(opciones.imgClass != null ? opciones.imgClass : opciones.type != null ? 'rup-feedback_image rup-feedback_image_' + opciones.type : '').attr({
				role: 'alert'
			}).css('display', opciones.block ? 'block' : 'none').css('visibility', 'hidden').append('<span class=\'rup-feedback-icon\'>').context.id;

			//Crear capa cierre

			var clase = $.rup.adapter[$.fn.rup_feedback.defaults.adapter].classComponent();
			opciones._divClose = $('<div />').html($.rup.adapter[$.fn.rup_feedback.defaults.adapter].closeLiteral()).attr('id', opciones._idFeedback + '_closeDiv').attr('title', $.rup.i18nParse($.rup.i18n.base, 'rup_feedback.closingLiteral')).addClass('rup-feedback_closeLink').addClass(clase);
     ...
     * @private
     */
		_addCloseLink: function _addCloseLink() {
			var opciones = this.options;
     ...
     */
		destroy: function destroy() {
			var opciones = this.options;
			this.element.removeClass().removeAttr('role').css('cssText', '').html('').stop().animate({
				opacity: '100'
     ...
     */
		set: function set(message, type, imgClass) {
     ...
     */
		hide: function hide(delay, fadeSpeed) {
			var opciones = this.options,
			    element = this.element;
